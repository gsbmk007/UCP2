...
void pushGameState(LinkedList* stack, int play_row, int play_col, int goal_row, int goal_col, int box_row, int box_col, char** array, int rows, int col) {
    /* Create a new GameState structure */
    GameState* game = createGameState(play_row, play_col, goal_row, goal_col, box_row, box_col, array, rows, col);

/*      Push the GameState onto the stack
 */    push(stack, game);
}

int popGameState(LinkedList* stack, int* play_row, int* play_col, int* goal_row, int* goal_col, int* box_row, int* box_col, char*** array, int* rows, int* col) {
    GameState* game;
    int i=0;
    pop(stack, (void**)&game);
