...
/* Old assignment 1 Code Not used in this program
Kept for reference  */
...
*box_col)
{
    int isset = 0;
       ...
       random coordinates if the coordinates are same as the player and the goal
coordinates
    ...
    *box_col = randomUCP(2, map_col - 1);
        if (!(((*box_row == goal_row) && (*box_col == goal_col)) || ((*box_row
== player_row) && (*box_col == player_col))))
        {
            isset = 1;
        }
    }
...
void plot_players(char **map_array, int player_row, int player_col,
                  int goal_row, int goal_col, int box_row, int box_col) {
  map_array[player_row][player_col] = 'P';
  map_array[goal_row][goal_col] = 'G';
  map_array[box_row][box_col] = 'B';
...
void remove_players(char **map_array, int player_row, int player_col,
                    int goal_row, int goal_col, int box_row, int box_col) {
